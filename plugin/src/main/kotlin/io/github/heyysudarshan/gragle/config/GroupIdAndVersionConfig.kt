...
internal fun Project.configureGroupIdAndVersion() {
    ...
    val groupId = libs.findVersion(groupIdIdentifier).orElseThrow {
        IllegalArgumentException(
            """
                To let Gragle set the groupId for the $name module automatically, add a version for `$groupIdIdentifier` in your version catalog.
    ...
    val version = libs.findVersion(versionIdentifier).orElseThrow {
        IllegalArgumentException(
            """
                To let Gragle set the groupId for the $name module automatically, add a version for `$versionIdentifier` in your version catalog.
                If you don’t want Gragle to do this, just set `assignGroupIdAndVersion = false` in the gragle {} block of your root build.gradle.kts file.
            """.trimIndent()
        )
    }
